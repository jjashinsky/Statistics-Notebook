...
<a href="javascript:showhide('ViewKids')">
<div class="hoverchunk">
<span class="tooltipr">
View(KidsFeet)
  <span class="tooltipRtext">Look at the section "View" above to understand first.</span>
</span><span class="tooltipr">
&nbsp;&nbsp;&nbsp;&nbsp;  
  <span class="tooltipRtext">Press Enter to run the code.</span>
</span><span class="tooltipr" style="float:right;">
...
<div id="ViewKids" style="display:none;padding-left:20px;">
```{r, echo=FALSE}
pander(KidsFeet)
```
</div>
...
`table( dataset$column1, dataset$column2)` 

* `dataset` is the name of your dataset while `column` is the name of one of the qualitative columns of that dataset. The table function then tallies up how many times each value of the quantitative variable occurs in that column.
* `column1` and `column2` are two different columns of the `dataset`. They should both be quantitative. The table function tallies how many times each possible combination of `column1` and `column2` occur in the `dataset`.
...
<div class="hoverchunk">
<span class="tooltipr">
## Note: the "KidsFeet" data set is in the "mosaic" library.
</span><span class="tooltipr">
library(mosaic)
  <span class="tooltipRtext">This loads the `KidsFeet` dataset, among other things, for use in these examples.</span>
</span>
</span>
</div>
<div class="hoverchunk">
<span class="tooltipr">
table(
  <span class="tooltipRtext">`table(` is an R function that is used to count how many times each value occurs in a specified column of data.</span>
</span><span class="tooltipr">
KidsFeet
  <span class="tooltipRtext">`KidsFeet` is a dataset. Type `View(KidsFeet)` to see it.</span>
</span><span class="tooltipr">
$
  <span class="tooltipRtext">`$` is used to pull the `birthyear` column from the `KidsFeet` dataset.</span>
</span><span class="tooltipr">
birthyear
  <span class="tooltipRtext">`birthyear` is one of the columns of the   `KidsFeet` dataset. It can be considered qualitative as birthyears are either 87 or 88.</span>
</span><span class="tooltipr">
)
  <span class="tooltipRtext">`)` closes the `table(` function.</span>
</span><span class="tooltipr">
&nbsp;&nbsp;&nbsp;&nbsp;
  <span class="tooltipRtext">Press Enter to run the code.</span>
</span><span class="tooltipr" style="float:right;">
...
```
```{r, echo=FALSE}
pander::pander(table(KidsFeet$birthyear))
```

Note how the `table()` function counts how many children have a birthyear of 1987 and how many had a birth year of 1988.
</div>
...
KidsFeet\$birthyear
  <span class="tooltipRtext">this pulls the qualitative variable `birthyear` from the `KidsFeet` dataset.</span>
</span><span class="tooltipr">
,
  <span class="tooltipRtext">`,` separates the two columns used in the table command.</span>
</span><span class="tooltipr">
KidsFeet\$birthmonth
  <span class="tooltipRtext">this pulls the qualitative variable `birthmonth` from the `KidsFeet` dataset.</span>
</span><span class="tooltipr">
)
  <span class="tooltipRtext">`)` closes the `table(` function.</span>
</span><span class="tooltipr">
...
```
```{r, echo=FALSE}
pander::pander(table(KidsFeet$birthyear,KidsFeet$birthmonth))
```

Note how the `table()` function counts how many children were born in each month of the year for 1987 as well as for 1988. This allows us to compare the birth month patterns accross the two different years.
</div>
...
The factor allows us to change the order of our factor-levels

```{r, eval=FALSE}
airquality$Month <- factor(airquality$Month, levels = c(5, 6, 7, 8, 9), ordered=TRUE)
```


To view the levels of factors
```{r, eval=FALSE}
levels(airquality$Month)
```

Views the structure of the data.frame
```{r, eval=FALSE}
str(airquality)
```

A quick way to change something to a factor
```{r, eval=FALSE}
as.factor(conc)
```

To remove any warning messages for a test, use this code. 
```{r, eval=FALSE}
{r, warning = FALSE}
```

To evaluate the probability in a normal distribution. 
```{r, eval=FALSE}
pnorm(q=.5, mean=0, sd=1)
pnorm(q=2, mean=0, sd=1) - pnorm(q=1, mean=0, sd=1)
```

To evaluate the x value under a normal distribution given you have the probability. 
```{r, eval=FALSE}
qnorm(p=.5, mean=0, sd=1)
```

This function allows you read a csv file and input it into a data frame. If headers exisit use header = TRUE. 
```{r, eval=FALSE}
regressionData <- read.csv("../Data/Rent.csv", header=TRUE)
```

This function allows you to find the amount of RGB in any color.
```{r, eval=FALSE}
col2rgb("blue")
```

To add a curve to a graph use this function.
```{r, eval=FALSE}
curve(exp(b[1]+b[2]*x)/(1+exp(b[1]+b[2]*x)), add=TRUE, col="blue")
```

To sum up specific dimensions in a table.The number is the dimension you wish to preserve. All other dimensions will be ignored and summed. 
```{r, eval=FALSE}
apply(Titanic, 1, sum)
...
To create a sequency plot of residuals 
```{r, eval=FALSE}
plot(carslm$residuals, type='b', pch=16, cex=0.8)
```
